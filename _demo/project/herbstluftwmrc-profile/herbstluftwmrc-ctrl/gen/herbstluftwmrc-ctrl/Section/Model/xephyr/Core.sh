...
mod_xephyr () {
	...
	mod_xephyr_run_herbstluftwm "$1" "$2"

}


mod_xephyr_run_herbstluftwm () {
	...
	## run herbstluftwm
	##DISPLAY=:100 herbstluftwm

	if [ "none$config_file_path" = "none" ]; then
		util_error_echo "DISPLAY=:100 herbstluftwm"
		DISPLAY=:100 herbstluftwm
	else
		util_error_echo "DISPLAY=:100 herbstluftwm -c $config_file_path"
		DISPLAY=:100 herbstluftwm -c $config_file_path
	fi
